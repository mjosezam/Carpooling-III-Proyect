package Graph;/* Generic Directed Weighted Graph with Dijkstra's Shortest Path Algorithm
...
public class DiGraph<T extends Comparable<T>>
{
    ...
    /**
     * Crea grafo con valores aleatorios
     * @return grafo aleatorio
     */
    public DiGraph crearGrafo(){
        DiGraph grafo = new DiGraph();
        Random random = new Random();
        String[] lugares = {"dulce_nombre","occidental","oriental","el_tejar","el_guarco","tierra_blanca","carmen","guadalupe",
                "san_nicolas","agua_caliente","turrialba","tucurrique","concepcion","san_rafael","cot","pacayas","cervantes","TEC"};
        for(int i = 0;i<40;i++){
            String desde = lugares[random.nextInt(18)];
            String hacia = lugares[random.nextInt(18)];
            if (desde == hacia){
                hacia = lugares[random.nextInt(18)];
            }
    ...
    /**
     * Realiza una busqueda Depth First a partir del primer vertice creado
     * @return true si esta conectado, false si no esta conectado
     */
